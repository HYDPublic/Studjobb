...
class TestBestSendDate(unittest.TestCase):
    ...
    def test_returns_monday_if_currently_sunday(self):
        ...
        self.assertEqual(remove_time(BestSendDate(now = sunday).date), monday)

    def test_returns_monday_if_currently_saturday(self):
        saturday = datetime.datetime(2015, 10, 03)
        monday   = datetime.datetime(2015, 10, 05)
        self.assertEqual(remove_time(BestSendDate(now = saturday).date), monday)
    ...
    def test_returns_tuesday_if_currently_monday_three_o_clock(self):
        ...
        best_date = BestSendDate(now = monday_three_o_clock).date.replace(hour=0, minute=0, second=0)
        self.assertEqual(best_date, tuesday)

    def test_returns_monday_if_currently_monday_two_o_clock(self):
        monday_two_o_clock = datetime.datetime(2015, 10, 05, 14, 00)
        monday = datetime.datetime(2015, 10, 05)
        best_date = BestSendDate(now = monday_two_o_clock).date.replace(hour=0, minute=0, second=0)
        self.assertEqual(best_date, monday)
    ...
    def test_returns_monday_if_currently_friday_three_o_clock(self):
        ...
        monday = datetime.datetime(2015, 10, 05)
        best_date = BestSendDate(now = friday_three_o_clock).date.replace(hour=0, minute=0, second=0)
        self.assertEqual(best_date, monday)
